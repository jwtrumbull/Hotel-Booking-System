		...
		checkIn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					stmt.executeUpdate("update customer set hotelName ='" + hotelEnter.getText() + "', roomNumber="
							+ roomEnter.getText() + " where cID=" + customerEnter.getText());
		...
		checkOut.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					stmt = conn.createStatement();
					stmt.executeUpdate(
							"update customer set hotelName = NULL, roomNumber=0 where cID =" + customerEnter.getText());

					stmt = conn.createStatement();
					stmt.executeUpdate("update room set reserved = 0 where roomNumber=" + roomEnter.getText());

					stmt = conn.createStatement();
					stmt.executeUpdate("delete from reservation where hotelName = '" + hotelEnter.getText()
							+ "' AND cID=" + customerEnter.getText());
				} catch (SQLException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					rs = stmt.executeQuery(
							"Select Customer.name, Customer.cID, Reservation.hotelName, Reservation.checkin From Customer LEFT JOIN Reservation ON Customer.cID = Reservation.cID");
					while (rs.next()) {
						str = str + "Customer Name: " + rs.getString("Customer.name") + "\nHotel Name: "
								+ rs.getString("Reservation.hotelName") + "\n-------------\n";
						rslt.setText(str);
					}
				} catch (SQLException e) {
					// TODO Auto-generated catch block
